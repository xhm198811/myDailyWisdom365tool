...
微信开发者工具 → 导入项目 → 目录选 `miniprogram`（**不要选仓库根目录**）→ 填自己的 AppID。

- ⚠️ **最容易卡住的一步**：详情 → 本地设置 → 勾选**「不校验合法域名、web-view（业务域名）、TLS 版本以及 HTTPS 证书」**。
  不勾选的话，请求会直接失败并只给出笼统的 `request:fail`，看不出原因。
  注意 `project.private.config.json` 里的 `urlCheck: false` **不代表工具 UI 里已经勾上**，两者可能不同步 —— 必须去 UI 里亲眼确认。
- 真机调试：`127.0.0.1` 无效，把 `config.js` 里 `dev.apiOrigin` 改成电脑局域网 IP（如 `http://192.168.0.10:8123`），
  后端用 `--host 0.0.0.0` 启动。**这个勾只对开发者工具生效，真机/体验版不认 http，正式发布必须走云托管 `container` 通道。**
